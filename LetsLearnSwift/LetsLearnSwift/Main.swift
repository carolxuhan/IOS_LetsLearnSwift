...
func distance(x1: Double, y1: Double, x2: Double, y2: Double) -> Double {
    let dist:Double = sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))
    return dist
}
...
func applesAndOranges(apples: Int, oranges: Int) -> String {
    var pluralApples = "apples"
    var pluralOranges = "oranges"
    if apples == 1{
        pluralApples = "apple"
    }
    if oranges == 1{
        pluralOranges = "orange"
    }
    return "I have \(apples) \(pluralApples) and \(oranges) \(pluralOranges)."
}
...
func vectorize(p1: Point, p2: Point) -> Vector {
    let changex = p2.x-p1.x
    let changey = p2.y-p1.y
    let result = Vector(dx: changex, dy: changey)
